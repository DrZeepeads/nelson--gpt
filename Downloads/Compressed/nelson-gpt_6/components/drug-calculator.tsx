...
export function DrugCalculator() {
  ...
  const [dosageHistory, setDosageHistory] = useState<Array<{ drug: string; dose: string; timestamp: string }>>([])

  const saveDosageHistory = (drug: string, dose: string) => {
    const newDosage = { drug, dose, timestamp: new Date().toISOString() }
    setDosageHistory((prevHistory) => [newDosage, ...prevHistory.slice(0, 9)])
  }
  ...
  const calculateDose = () => {
    ...
    if (isNaN(weightNum) || weightNum <= 0 || !selectedCategory || !selectedDrug) {
      toast({
        title: "Invalid Input",
        description: "Please enter a valid weight and select a drug.",
        variant: "destructive",
      })
      return
    }
    ...
    const result = `Recommended dose: ${dose.toFixed(2)} ${drug.unit}`
    setResult(result)
    saveDosageHistory(selectedDrug, result)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-8">
        <h3 className="text-xl font-semibold mb-4">Dosage History</h3>
        {dosageHistory.length > 0 ? (
          <ul className="space-y-2">
            {dosageHistory.map((dosage, index) => (
              <li key={index} className="bg-gray-100 p-2 rounded">
                <strong>{dosage.drug}:</strong> {dosage.dose}
                <br />
                <small>{new Date(dosage.timestamp).toLocaleString()}</small>
              </li>
            ))}
          </ul>
        ) : (
          <p>No dosage history available.</p>
        )}
      </div>
