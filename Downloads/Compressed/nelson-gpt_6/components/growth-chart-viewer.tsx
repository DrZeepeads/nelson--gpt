...
export function GrowthChartViewer() {
  ...
  const handlePlotPoint = () => {
    if (age && measurement) {
      ...
      if (chartType === "weight-for-age" && chartData.some((d) => d.age === parsedAge && d.height)) {
        const height = chartData.find((d) => d.age === parsedAge)!.height!
        const calculatedBMI = calculateBMI(parsedMeasurement, height)
